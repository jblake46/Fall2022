...
{
    //swawps p with the next node in place
    //sets previous to point to p
    p->next->next->prev = p;
    //makes the next node after p, that is pointing to the previous
    //node, to point to the previous memory address
    p->next->prev = p->prev;
    //makes p pointing to prev pointing to next, to point to the next node
    p->prev->next = p->next;
    //swapping p prev and p next
    p->prev = p->next;
    p->next = p->next->next;
    p->prev->next = p;
}
