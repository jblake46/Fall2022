...
class Grouping
{
    ...
    public:
        ...
        vector<GridSquare> findGroup(int r, int c, vector<GridSquare> groupMap); //Implement this function (recursive)
        void printGroups(); //Displays grid's groups of squares
        vector<vector<GridSquare>> getGroups(); //Needed in unit tests
        //bool notUsedYet(int row, int col);
        bool findGroupHelper(int, int, vector<GridSquare>);
};