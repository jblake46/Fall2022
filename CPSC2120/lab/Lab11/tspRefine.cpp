/*
 * Name: Blake Moore
 * Date Submitted: 12/15
 * Lab Section: 2:30 Section
 * Assignment Name: Traveling Salesmen
 */
...
bool refine(double &len)
{
  ...
  //go through the each pair of edges
  for(int i = 0; i < P.size(); i ++)
  {
    for(int j = 0; j < P.size(); j++)
    {
      ...
      distance2 = dist(i,j) + dist(i+1, j+1);
      distance = dist(j, j+1) + dist(i,i+1);
      double diff = distance - distance2;
  ...
  //returns false if if statement is not executed
  return false;
}
...
double tspRefine()
{
  ...
  //using shuffle to randomly generate tour
  shuffle(P.begin(), P.end(), rand());

  for(int i = 0; i<P.size(); i++)
  {
    lengthOfRoute += dist(i, i+1);

    while(refine(lengthOfRoute))
    {
      //empty while loop bc we want it to refine
      //until refine returns false
    }
    if(lengthOfRoute < best_len)
    {
      best_len = lengthOfRoute;
      best = P;
